...
def wait_for_server(base_url: str, timeout: float = 60.0) -> bool:
    ...
    while time.monotonic() < deadline:
        try:
            data = _request_json(f"{base_url}/api/health", timeout=2.0)
            if data is not None:
                return True
...
def wait_for_snapshot_readiness(
    ...
    page of ``jobs``, so it works for 10k/50k-job workspaces. The snapshot
    endpoint requires an authenticated session (workspace access guard).
    """
    deadline = time.monotonic() + timeout
