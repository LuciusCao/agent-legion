...
ID_PATTERN: re.Pattern[str] = re.compile(
    r"^(API|DB|EXEC|RECOVERY|BOUNDARY|SECURITY|CONFIG|VAULT|STUDIO|MATERIAL|RUN|CAMPAIGN)-[A-Z]+(-[A-Z]+)*-[0-9]{3}$"
)
...
def _is_in_gate(target: str, gate: str) -> bool:
    ...
    The quick gate shares its ``tests/`` prefix with the more specific full and
    ci_extended gates; targets under those must not masquerade as quick evidence.
    """
    if not any(target.startswith(prefix) for prefix in GATE_PREFIXES[gate]):
        return False
    if gate != "quick":
        return True
    specific = tuple(p for o, ps in GATE_PREFIXES.items() if o != gate for p in ps)
    return not any(target.startswith(p) for p in specific)
