...
Usage:
    uv run python -m scripts.backfill_failure_classification [--dry-run]
"""
...
def backfill_failure_classification(
    ...
    with read_connection(database_dsn) as conn:
        rows = conn.execute(
            """
            select id, exit_code, error_message
            from node_runs
    ...
    planned = [
        (int(row["id"]), *classify_failure(row["exit_code"], str(row["error_message"])))
        for row in rows
    ]
    if dry_run:
    ...
    with write_transaction(database_dsn) as conn:
        for run_id, category, detail in planned:
            cursor = conn.execute(
                ...
                where id=? and status='failed' and failure_category=''
                """,
...
def main() -> None:
    ...
    matched, updated = backfill_failure_classification(dsn, dry_run=args.dry_run)
    if args.dry_run:
