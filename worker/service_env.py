...
from worker.proxy_config import redact_proxy_url
...
def strip_proxy_env() -> None:
    ...
    for name in stripped:
        del os.environ[name]
    if stripped:
        # print 而非 logger：剥离发生在 uvicorn 起来之前，root logger 尚未
        # 配置（全 repo 无 basicConfig，INFO 会被静默丢弃），这行是 runbook
        # 给运维的排障标记，必须保证可见；flush 对齐 executor 的 stdout 风格。
        print(
            f"已剥离继承的代理环境变量（默认直连；如需代理请在配置中设置 proxy）: "
            f"{', '.join(stripped)}",
            flush=True,
        )
...
def proxy_env_overrides(proxy: object) -> dict[str, str]:
    ...
    if not url:
        return {}
    print(
        f"按配置注入出网代理（executor 与 agent 子进程经代理出网）: {redact_proxy_url(url)}",
        flush=True,
    )
