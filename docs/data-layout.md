...
| `bin/` | Worker 自带二进制 | 原生/裸机形态不再有常规内容（#507 起 velites 装到 PATH——`~/.local/bin`，由 `scripts/ensure-velites.sh` 无参形态安置并按指纹维护；存量 `bin/velites` 旧副本可删，`make install` 检测到会提示）。Docker 形态是本目录的存在理由：agent runtime 执行器不进 worker 镜像（issue #381），compose 把平台匹配的二进制 bind mount 到 `/app/data/bin/velites`（即镜像内的此目录） | Worker 二进制解析顺序：PATH 优先、自带副本兜底（`worker/binary_resolution.py::resolve_binary`，#507）；容器内 PATH 上无 velites、兜底即命中，裸机以 PATH 的机器级单副本为准 |
