...
- **裸机/开发部署**（直接跑 `worker.executor`，如 `make dev-worker`）：在**与 Worker 同 OS/架构**的机器上、仓库根执行 `./scripts/ensure-velites.sh`（无参形态），脚本按 velites/ 源码指纹决定是否需要 `cargo build --release`（指纹不变的重复执行直接跳过），产物原子安装到 `~/.local/bin/velites`（`VELITES_INSTALL_DIR` 可覆盖）——机器级单一副本，PATH 命中即解析；调用方 PATH 不含该目录时脚本会向 stderr 打「把目录加入 PATH」的指引（不阻断——`native-prod-up.sh` 与 `dev_stack.sh` 启动服务前已自动把该目录前置到服务进程的 PATH，目录经 `ensure-velites.sh --print-bin-dir` 查询、判定逻辑单一事实源，服务链不受用户 shell 的 PATH 影响；交互 shell 则需自行把目录写入 profile）。`make prod-up`（原生形态）每次启动前同样经它刷新。无源码/工具链的机器可直接取 Release 产物安置到 `~/.local/bin/`（或在 PATH 上的任意目录）。**存量 data/bin 旧副本**（旧版 `make install` 曾播种、之后无人维护）：#507 起不再遮蔽 PATH，但建议删除——`make install` 检测到时会打印删除指引（`rm -f data/bin/velites data/bin/velites.src-stamp`）。macOS 产物用 seatbelt、Linux 产物用 bubblewrap（Linux 主机需可用的 bwrap：setuid 或非特权 user namespace），沙箱后端不可用同样 fail-closed。裸机部署同样可设 `AGENT_WORKER_EXPECT_RUNTIMES`（如 systemd 单元的 `Environment=`）启用期望 runtime 守卫；不设时保持「探测到什么声明什么」的默认语义。
