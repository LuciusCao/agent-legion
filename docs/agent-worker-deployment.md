...
原生形态（`make prod-up` 不带 `docker` 参数）没有 compose 端口发布层，对应开关是 `NATIVE_BACKEND_BIND` / `NATIVE_WORKER_BIND`（默认 `127.0.0.1`），设为部署机局域网/overlay 网络 IP 即对其他设备暴露 Host API 与 Worker 控制台。这组变量（连同端口覆盖 `NATIVE_BACKEND_PORT` / `NATIVE_WORKER_PORT`）按「进程环境 > 根 `.env`」两级来源读取：持久配置写 `.env`（换 shell 会话、重启或经 launchd/cron 非 shell 调起时不再静默退回 loopback），临时覆盖直接在命令前导出环境变量（优先于 `.env`，`NATIVE_BACKEND_BIND=0.0.0.0 make prod-up` 一次性生效）。幂等判定按「bind + 端口 + 地址族」匹配既有监听：通配 bind（`0.0.0.0` / `::`）请求且同端口已有具体地址监听时，脚本视为已在运行并跳过启动（不并行起第二个实例连同一个库——单副本约束下会造成 SSE 分裂/限速稀释/暂停互踩），如需切换 bind 请先 `make prod-down`。切换 bind 的完整迁移流（`.env` 已持久携带新 bind 时）：旧实例绑的具体地址不再被 `make prod-down` 命中（down 按新 bind 定位），落到「仍有监听但绑定形态不同、未停止」的 rc 1 分支——此时用旧地址做环境覆盖停掉残留实例，再按新 bind 起：`NATIVE_BACKEND_BIND=<旧地址> make prod-down`（Worker 残留同理用 `NATIVE_WORKER_BIND`）；环境变量优先于 `.env` 且一次性生效，随后正常 `make prod-up` 即以新 bind 启动。对象存储的端口发布仍由 compose 托管，`AGENT_LEGION_S3_BIND` 对两种形态同样生效：绑定为具体 IP 时 `127.0.0.1` 映射消失，原生后端进程访问 S3 的 `AGENT_LEGION_S3_ENDPOINT`（根 `.env`，默认 `http://127.0.0.1:8333`）需同步指向该地址，或把 `AGENT_LEGION_S3_BIND` 设为 `0.0.0.0` 保住 loopback；远程客户端的 `AGENT_LEGION_S3_PUBLIC_ENDPOINT` 一并指向可达地址（完整说明见 [materials-storage-deployment.md](materials-storage-deployment.md)）。
...
（PATH 优先、自带副本 `data/bin/` 兜底——后者是 Docker 外挂形态的注入点，#507
起原生形态只装 PATH）探测已安装的 runtime 并默认全部启用，
...
**velites 二进制来源（Worker 自带沙箱）**：Worker 解析 velites 的顺序是「PATH 优先，自带副本 `<仓库根>/data/bin/velites` 兜底」（issue #507：原生形态收敛为 PATH 单一副本、由 `ensure-velites.sh` 的指纹闸门维护——开发 worktree 维护机器级 `~/.local/bin`，生产 worktree 维护按 worktree 派生的隔离目录，见下文裸机小节；data/bin 的兜底语义只服务 Docker 外挂形态），启动预检与 code 执行共用同一解析逻辑；两处都找不到才 fail-closed。worker 镜像**不含任何 agent runtime 执行器**（issue #381）——velites 与 pi 都由部署方以外挂二进制提供，本机装什么 runtime 就声明什么：

- **Docker 部署**：从 GitHub Release（`velites-v*` tag，velites-release workflow 产出）下载与宿主机架构一致的 tarball，解出的 `velites` 放到 compose 的 `VELITES_BIN`（默认 `../velites-bin/velites`）——compose 把它 bind mount 到容器内 `/app/data/bin/velites`（自带副本目录；容器内 PATH 上只有镜像内置的 velites-sandbox，data/bin 是 runtime 二进制的唯一来源，兜底即命中）。架构必须与 worker 镜像一致（x86_64 取 `*-x86_64-unknown-linux-gnu`，arm64 取 `*-aarch64-unknown-linux-gnu`）；挂载了错误架构的二进制能通过存在性探测，但执行时以 exec format error 失败——期望 runtime 守卫会把它转成启动失败（见下）。**防漏挂载守卫**：compose 默认注入 `AGENT_WORKER_EXPECT_RUNTIMES=velites`（`deploy/.env` 可覆盖：多值逗号分隔；显式置空禁用守卫，零 runtime 注册合法——零 runtime / 纯 code 池形态同时叠加 `deploy/compose.worker.zero-runtime.yaml` override 去掉 velites bind mount，否则无条件挂载会要求准备一个用不上的二进制文件），启动时探测不到期望 runtime、或期望 runtime 模型发现失败（含架构错配）即 fail-fast（退出码 2，supervisor 不自动重启、healthcheck 变 unhealthy）。**pi 在 docker 镜像内不可用**：pi 的入口是 npm 包脚本，依赖 node 运行时与包树，而 #381 已把它们移出镜像——pi 部署走裸机形态（PATH），需要在 docker 跑 pi 时自行构建含 node+pi 的镜像变体；
- **裸机/开发部署**（直接跑 `worker.executor`，如 `make dev-worker`）：在**与 Worker 同 OS/架构**的机器上、仓库根执行 `./scripts/ensure-velites.sh`（无参形态），脚本按 velites/ 源码指纹决定是否需要 `cargo build --release`（指纹不变的重复执行直接跳过），产物原子安装到 `~/.local/bin/velites`（`VELITES_INSTALL_DIR` 可覆盖）——开发 worktree 语义下的机器级单一副本，PATH 命中即解析。**生产 worktree 隔离**（dev 与 prod worktree 同机共存的污染防护，PR #519）：在 `.worktrees/prod` 里执行时（或任何 worktree 显式设 `AGENT_LEGION_VELITES_ISOLATED=1`），改装到按 worktree 名派生的隔离目录 `${XDG_DATA_HOME:-~/.local/share}/agent-legion/<worktree>/bin`，且不读 PATH 探测与 `VELITES_INSTALL_DIR`——两者是全 worktree 共享的通道（shell profile），正是「一次开发安装改写生产在用二进制」的污染路径；`=0` 显式退回机器级共享语义（逃生口）。开发侧反向同理：PATH 解析到的副本若位于隔离目录树（其他 worktree 的私有运行时），开发安装不维护它，回落共享目录。调用方 PATH 不含安装目录时脚本会向 stderr 打指引（不阻断——`native-prod-up.sh` 与 `dev_stack.sh` 启动服务前已自动把该目录前置到服务进程的 PATH，目录经 `ensure-velites.sh --print-bin-dir` 查询、判定逻辑单一事实源，服务链不受用户 shell 的 PATH 影响；交互 shell 则需自行把目录写入 profile，prod 隔离目录不建议写入 profile——会让开发 shell 也解析到生产副本）。`make prod-up`（原生形态）每次启动前同样经它刷新。prod worktree 首次改用隔离目录时，`~/.local/bin` 的既有共享副本不代删（开发侧继续维护，这台机器已无开发 worktree 在用时才是无主文件），脚本 stderr 提示可手动清理。无源码/工具链的机器可直接取 Release 产物安置到 `~/.local/bin/`（或在 PATH 上的任意目录）。**存量 data/bin 旧副本**（旧版 `make install` 曾播种、之后无人维护）：#507 起不再遮蔽 PATH，但建议删除——`make install` 检测到时会打印删除指引（`rm -f data/bin/velites data/bin/velites.src-stamp`）。macOS 产物用 seatbelt、Linux 产物用 bubblewrap（Linux 主机需可用的 bwrap：setuid 或非特权 user namespace），沙箱后端不可用同样 fail-closed。裸机部署同样可设 `AGENT_WORKER_EXPECT_RUNTIMES`（如 systemd 单元的 `Environment=`）启用期望 runtime 守卫；不设时保持「探测到什么声明什么」的默认语义。
