...
- skill 注入（显式 `--skill` 目录）+ `read`/`write`/`bash` 核心三工具，外加少量
  opt-in 的确定性工具（§8，#442 起：`uuid`）——只收跨 skill 通用原语，不收业务逻辑，
  不演变为插件机制（非目标不变）；
...
- **uuid（#442，opt-in）**：`generate`（`count` ≤100、`version` v4 随机 / v7 时间有序）
  与 `validate`（逐值报告格式/版本/variant，任一非法即 `is_error`）。模型手写 UUID
  是经典翻车点（假 hex、错误版本位），生成必须走工具。`generate` 是非确定性操作
  （replay/重跑结果不同），工具描述要求模型把生成的值落进产物文件而非指望可复现。
  不触碰文件系统与沙箱；默认不进 `--tools`（核心三件套之外按需启用，避免把
  runtime 不认识的工具名透传给 pi）；
