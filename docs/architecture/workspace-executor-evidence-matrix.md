...
| Video detail GET is read-only; cache persistence occurs only on explicit write/event paths | route/service | BOUNDARY-READ-001 | `tests/test_api.py::test_video_detail_read_only_does_not_update_video`, `tests/test_video_read_service.py` | N/A | Verified | N/A |
| Workspace runtime stats derive from executor allocations, bindings, leases, and capacities | executor/service | EXEC-STATS-001 | `tests/test_workspace_configuration_service.py::test_executor_stats_report_configured_capacity_and_leases`, `tests/test_workspace_configuration_service.py::test_executor_stats_does_not_consult_agent_status_manager`, `tests/test_jobs_api.py::test_workspace_stats_executor_status_reflects_allocations_and_leases` | N/A | Verified | N/A |
...
| Executor global capacity is shared across workspaces and never exceeded | executor | EXEC-CAPACITY-001 | `tests/test_pipeline_worker_concurrency.py::test_global_capacity_enforced_by_lease_transaction` | `tests/full/test_executor_worker_fairness.py::test_shared_capacity_and_bounded_fairness` | Verified | N/A |
| Runnable workspaces receive bounded scheduling opportunity under contention | scheduler/worker | EXEC-FAIRNESS-001 | `tests/test_pipeline_worker_concurrency.py::test_round_robin_allows_small_workspace_to_claim` | `tests/full/test_executor_worker_fairness.py::test_shared_capacity_and_bounded_fairness` | Verified | N/A |
