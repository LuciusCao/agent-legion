...
| Runtime queues and job event sequence numbers are persisted in PostgreSQL; no new in-process authoritative runtime state | database | DB-STATE-001 | `tests/executors/test_remote_broker_persistence.py`, `tests/test_job_event_buffer_db.py`, `tests/ci/test_postgres_only_runtime.py` | N/A | Verified | N/A |
...
| PostgreSQL schema initialization is transactional, advisory-locked, and idempotent | database/migration | RECOVERY-MIGRATION-001 | `tests/db/test_postgres_runtime.py::test_schema_initialization_is_idempotent` | `tests/full/test_postgres_agent_concurrency.py` | Verified | N/A |
...
| Remote worker credentials are per-worker and revocable; per-worker tokens persist only as sha256 hashes of the secret part, revocation is immediate and beats the static-token fallback window | security/remote | SECURITY-WORKER-001 | `tests/remote/test_worker_tokens.py` | `tests/full/test_worker_token_security.py` | Verified | N/A |
| Shard executions are claimed through ExecutorLeaseRepository.try_claim with one lease per shard; fan-out never bypasses the capacity system | executor/scheduler | EXEC-SHARD-001 | `tests/workflows/test_sharding.py` | N/A | Verified | N/A |
