...
depends on the mounted velites. Bare-metal deployments install velites onto
PATH (`./scripts/ensure-velites.sh`, fingerprint-gated rebuild into
`~/.local/bin`, issue #507; same OS/arch as the Worker) for both roles.
...
- **Worker declarations first, definition migration second.** A queued request
  ...
  supported runtime's binary (PATH first, then the bundled `data/bin/<binary>`
  — the Docker mount point; issue #507) and enables every detected one by default — installing the velites binary
...
| Worker exits with code 2 and logs `启动预检失败` / startup preflight failure | `max_code_concurrency > 0` without a resolvable `velites` binary (agent runtimes are auto-detected since issue #254 and can no longer fail preflight) | Install velites on PATH (`./scripts/ensure-velites.sh`, per-platform, installs to `~/.local/bin`; issue #507) — or set `max_code_concurrency: 0`, then restart |
