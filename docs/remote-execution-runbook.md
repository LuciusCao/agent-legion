...
| 高并发档位下运行容量规律性锯齿：贴满上限 → 数分钟一次掉 10%–20% 并一两分钟回满，worker 侧上传队列同时排队 | Host 单进程控制面在完成波下饱和（#521）：DAG 同相位节点成波报告，result commit 的 GIL 绑定工作（tar 解包、产物校验、写事务、events 后处理）打满单核，claim/心跳被饿死 | 运行画像（`/api/metrics/runtime-profile`）的 result 分段列（schema v80 起：`result_unpack / artifacts_verify / validate / artifacts_upload / lease_write / events / mark_done_seconds_total/max`）指认吃 CPU 的段；`result stages:` 日志行（超过 `AGENT_LEGION_SLOW_RESULT_MS`，默认 15s，升 WARNING）给单次分解。削峰 gate 默认已开（`agent_workers.max_concurrent_result_commits` = 16，instance settings 可调，0 = 关闭做 A/B）；遗留绝对路径警告应已由一次性清理归零（启动报告 `report_absolute_db_paths` 全零），仍在刷说明有不可映射行留在库里。#521 方案 B（角色拆分）落地后：削峰 gate + 打点仍生效于 HTTP 平面，但根治手段是把调度面挪进专用进程（`AGENT_LEGION_HOST_ROLE` http + scheduler 双平面，见 deployment.md 单副本约束节）——若仍在 combined 单进程形态遇到本症状，迁移到双平面后 claim/心跳不再与 result commit 竞争同一 Python 进程 |
