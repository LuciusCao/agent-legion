...
| 心跳 relay 日志行（`worker/heartbeat_relay.py`，进 worker 控制台与滚动日志 `data/logs/executor.log`） | Worker | 「暂停心跳 relay——租约将按 Host TTL 正常过期重排」= executor 主循环超 60s 未刷新租约快照（进程卡死级饱和），relay 主动停拍让租约按 TTL 过期重排（保守方向）；「心跳 relay 批量拍失败」= Host 不可达，逐拍重试。排查卡点看 executor 滚动日志（10MB×5 轮转） |
