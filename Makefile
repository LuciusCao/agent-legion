...
dev-backend: ## 启动后端开发服务器 (127.0.0.1:$(DEV_BACKEND_PORT))
	ulimit -n $(DEV_NOFILE_LIMIT); $(UV) run uvicorn server.app.main:create_prod_app --factory --reload --reload-dir server --timeout-graceful-shutdown 3 --host 127.0.0.1 --port $(DEV_BACKEND_PORT) --log-config deploy/uvicorn-log-config.json
...
.PHONY: stack-host-up
stack-host-up: ## 部署机：启动 PostgreSQL + Agent Legion Host + 本机 Worker（本地对象存储由 local-s3-decide.sh 按 AGENT_LEGION_LOCAL_S3/BACKEND 决策，skip 时输出为空）
