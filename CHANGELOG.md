...
- skill 无契约块时跳过 velites spawn（issue #538，hot-fix）：#521 落地的
  ...
  （`velites-sandbox validate`）每次 result 都 spawn 一轮，而生产 skill
  盘点（11/11）显示 `references/output-contract.md` 普遍只有 prose 与
  `json` 示例围栏、0/11 携带 `yaml contract` 机器可读契约块，引擎空转
  （mode=existence 无裁决）纯耗服务时间，波峰排队下 validate 段上秒。
