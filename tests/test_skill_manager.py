...
# --- git argument-injection guards (defense-in-depth for the admin-managed
# skill documents): option-shaped repo/ref/commit values must fail validation
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "--config=core.fsck=evil",
    ],
)
def test_option_shaped_repo_is_rejected_before_git(tmp_path: Path, repo: str) -> None:
    manager = _make_manager(tmp_path, _single_skill(repo))
    with pytest.raises(SkillConfigError, match="skill repo"):
        manager.get_skill_dir(_KEY, str(uuid.uuid4()))


def test_exotic_url_scheme_repo_is_rejected(tmp_path: Path) -> None:
    manager = _make_manager(tmp_path, _single_skill("ext::sh -c whoami"))
    with pytest.raises(SkillConfigError, match="forbidden git transport"):
        manager.get_skill_dir(_KEY, str(uuid.uuid4()))
...
def test_option_shaped_or_invalid_ref_is_rejected(tmp_path: Path, ref: str) -> None:
    repo_uri = _make_bare_repo(tmp_path)
    manager = _make_manager(tmp_path, _single_skill(repo_uri, ref))
    with pytest.raises(SkillConfigError, match="not a valid git refname"):
        manager.get_skill_dir(_KEY, str(uuid.uuid4()))


def test_option_shaped_locked_commit_is_rejected(tmp_path: Path) -> None:
    """The DB lock document path: a non-hex commit fails before cat-file."""
    repo_uri = _make_bare_repo(tmp_path)
    store = memory_skill_store(_single_skill(repo_uri))
    store.put_lock(
        SkillsLock(
            skills={_KEY: LockedSkillSource(repo=repo_uri, ref="main", commit="--exec=evil")}
        )
    )
    manager = SkillManager(store=store, base_dir=tmp_path / "skills", runs_dir=tmp_path / "runs")
    with pytest.raises(SkillConfigError, match="not a hex object id"):
        manager.get_skill_dir(_KEY, str(uuid.uuid4()))


def test_legitimate_repo_and_ref_shapes_still_pass(tmp_path: Path) -> None:
    """The guards must not reject the shapes real sources use."""
    from server.app.skills.manager import _validate_commit, _validate_ref, _validate_repo

    assert _validate_repo("https://example.com/skills.git") == "https://example.com/skills.git"
    assert _validate_repo("git@github.com:acme/skills.git") == "git@github.com:acme/skills.git"
    assert _validate_repo("/opt/skills") == "/opt/skills"
    assert _validate_ref("main") == "main"
    assert _validate_ref("refs/tags/v1.2.3") == "refs/tags/v1.2.3"
    assert _validate_ref("v1.2.3") == "v1.2.3"
    assert _validate_commit("abc123def456") == "abc123def456"
    assert _validate_commit("a" * 40) == "a" * 40
