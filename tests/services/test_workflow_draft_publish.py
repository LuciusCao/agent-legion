...
nodes:
  do_thing:
    ...
    config:
      token:
        secret_ref: node:test_publish_flow:do_thing:token
"""


def test_publish_rejects_plaintext_secret_config(tmp_path: Path) -> None:
    """VAULT-SECRET-001: secret-marked node config must be a vault ref marker.

    The definition is persisted verbatim and its config is frozen into every
    future job, so a plaintext value here would persist outside the vault.
    """
    queries = JobQueries(TEST_DATABASE_URL, tmp_path / "jobs")
    workspace = _workspace(queries)
    _seed_node_code(workspace["id"])

    ok, errors = publish_workflow_draft(queries, workspace["id"], _PLAINTEXT_SECRET_YAML)

    assert ok is False
    assert any("secret_ref" in error and "do_thing" in error for error in errors)
    assert queries.get_active_workflow_revision(workspace["id"], "test_publish_flow") is None


def test_publish_accepts_secret_ref_marker_config(tmp_path: Path) -> None:
    queries = JobQueries(TEST_DATABASE_URL, tmp_path / "jobs")
    workspace = _workspace(queries)
    _seed_node_code(workspace["id"])

    ok, errors = publish_workflow_draft(queries, workspace["id"], _REF_SECRET_YAML)
