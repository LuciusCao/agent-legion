...
def _seed_mixed(job_db, tag: str) -> dict[str, Any]:
    ...
    job_db.update_job_node(running["id"], "write_script", status="running")
    _add_active_lease(job_db, leased, "write_script", f"lease-{tag}")
    job_db.update_job_node(failed_node["id"], "write_script", status="failed")
...
def test_batch_rerun_matches_per_job_rerun_node_mode(rerun_service, job_db) -> None:
    ...
    expected = _per_job_results(
        rerun_service, dataset_a["ws_id"], dataset_a["node_mode_ids"], "write_script"
    )

    dataset_b = _seed_mixed(job_db, "B")
    actual = rerun_service.batch_rerun(
        dataset_b["ws_id"], dataset_b["node_mode_ids"], "write_script"
    )
...
def test_batch_rerun_matches_per_job_rerun_from_failed_node(rerun_service, job_db) -> None:
    ...
    assert actual[0]["node_key"] == "write_script"
...
def test_category_batch_matches_per_job_rerun_targets(rerun_service, job_db) -> None:
    ...
    _fail_run(job_db, technical_job, "write_script", "technical", "model_error")
    _fail_run(job_db, business_job, "write_script", "business", "empty_content")
...
def test_batch_rerun_read_queries_bounded(rerun_service, job_db, monkeypatch) -> None:
    ...
    results = rerun_service.batch_rerun(ws_id, ids, "intake_knowledge_points")
