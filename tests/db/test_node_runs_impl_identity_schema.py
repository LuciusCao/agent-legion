"""node_runs 实现身份记录的写入路径单测（issue #645，schema v86）。
...
播种端到端的主/漂移用例见
``tests/services/test_job_workflow_upgrade_inherit_codex4.py``（v86 段）。
"""
...
def test_local_pool_claim_persists_impl_hash(queries: JobQueries) -> None:
    ...
    code 池执行从此有可证明的实现身份记录（v86 前恒「不可证明」）。
    """
...
def test_latest_done_request_identities_empty_run_hash_falls_back(job_db) -> None:
    """node_runs 身份为空串（v86 前的 run 行）→ 段 1 不命中，请求行兜底。"""
    workspace_id = "test-workspace"
    with job_db.connect() as conn:
        conn.execute(
            "insert into workspaces(id, name, default_workflow_key)"
    ...
    # run 行不带身份（v86 前形态），请求行有 done 记录。
