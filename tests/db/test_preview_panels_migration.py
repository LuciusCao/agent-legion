...
def test_upgrade_from_v70_applies_the_widening() -> None:
    ...
    # 75 the chain tail is node_runs_skill_key: init_db's high-water skip
    # means the test must drop v71 AND every later version (75 included) to
    # force the replay; the extra migrations' applies are idempotent.
    assert SCHEMA_VERSION == 75
    assert MIGRATIONS[-1].name == "node_runs_skill_key"
    with write_transaction(TEST_DATABASE_URL) as conn:
