...
def test_upgrade_from_v70_applies_the_widening() -> None:
    ...
    # 76 the chain tail is studio_publish_requests: init_db's high-water skip
    # means the test must drop v71 AND every later version (76 included) to
    # force the replay; the extra migrations' applies are idempotent.
    assert SCHEMA_VERSION == 76
    assert MIGRATIONS[-1].name == "studio_publish_requests"
    with write_transaction(TEST_DATABASE_URL) as conn:
