...
def test_workflow_catalog_loads_workflow_definition(settings):
    service = WorkflowCatalogService(settings)
    payload = service.workflow("education_video_problems_generation")
    assert payload["key"] == "education_video_problems_generation"
...
def test_workflow_catalog_payload_matches_revision_serializer(settings):
    service = WorkflowCatalogService(settings)
    payload = service.workflow("education_video_problems_generation")
    definition = service.definition("education_video_problems_generation")
