...
def test_fresh_install_workspace_and_revision_flow(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        ...
        assert workspace_id == "education_video_problems_generation"
...
def test_create_workspace_and_scoped_jobs_when_enabled(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        workspace_response = c.post(
            "/api/workspaces",
            json={"id": "math_sprint", "name": "Math Sprint"},
        )
...
def test_delete_workspace_rejects_when_jobs_running(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        ws = c.post(
            "/api/workspaces",
            json={"id": "running_ws", "name": "Running WS"},
        ).json()
        ws_id = ws["workspace"]["id"]
        ...
        c.post(
            ...
            json={
                "workflow_key": "running_ws",
                "source_kind": "direct_ids",
                "knowledge_point_ids": ["Q501"],
            },
        )
        job_db = app.state.job_db
        job_id = f"{ws_id}_running_ws_Q501"
...
def test_delete_workspace_cascades_and_returns_deleted_id(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        ws = c.post(
            "/api/workspaces",
            json={"id": "delete_me", "name": "Delete Me"},
        ).json()
        ws_id = ws["workspace"]["id"]
        publish_legacy_intake_revision(c.app.state.job_db, ws_id)
        c.post(
            f"/api/workspaces/{ws_id}/job-batches",
            json={
                "workflow_key": "delete_me",
                "source_kind": "direct_ids",
                "knowledge_point_ids": ["Q601"],
            },
        )
        job_db = app.state.job_db
        job_id = f"{ws_id}_delete_me_Q601"
...
def test_create_workspace_rejects_bad_and_duplicate_ids(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        ...
        first = c.post("/api/workspaces", json={"id": "dup_ws", "name": "First"})
        conflict = c.post("/api/workspaces", json={"id": "dup_ws", "name": "Second"})

    assert first.status_code == 200
    assert conflict.status_code == 409
    assert "exists" in conflict.json()["detail"].lower()


def test_create_workspace_rejects_retired_fields(tmp_path):
    """workflow_mode / default_workflow_key are retired (extra=forbid)."""
    from fastapi.testclient import TestClient
    ...
    with authenticate_client(TestClient(app)) as c:
        response = c.post(
            "/api/workspaces",
            json={"id": "legacy", "name": "Legacy", "workflow_mode": "demo"},
        )
...
def test_update_workspace_intake_config(tmp_path):
    ...
    with authenticate_client(TestClient(app)) as c:
        created = c.post(
            "/api/workspaces",
            json={"id": "update_intake", "name": "Update Intake"},
        ).json()
