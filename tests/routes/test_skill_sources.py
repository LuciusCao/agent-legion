...
KEY = "education-video-problems-generation/write-script"
...
def test_get_merged_view_after_seed(client) -> None:
    ...
    assert len(skills) == len(BUILTIN_SKILL_SOURCES.skills) == 4
    source = BUILTIN_SKILL_SOURCES.skills[KEY]
    entry = next(item for item in skills if item["key"] == KEY)
    # Demo skills are intentionally unlocked until the first relock; the lock
    # document metadata (resolved_at) still comes from the seeded lock.
    assert entry == {
        "key": KEY,
        "repo": source.repo,
        "ref": source.ref,
...
def test_put_updates_source_and_marks_stale(client) -> None:
    ...
    # The lock has no entry for the demo skills, so everything is stale until
    # relock.
    assert changed["locked_commit"] is None
    assert changed["stale"] is True
    assert all(item["stale"] for item in skills)
