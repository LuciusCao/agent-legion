...
Production wiring: the router is included by create_studio_agent_tools_router
(studio_agent_tools.py), so these tests run against the real app. The guard
matrix mirrors test_studio_agent_tools.py: anonymous 401, full session 403,
scoped tokens pass the scope guard, workspace-bound tokens get 403 on other
workspaces. The action-surface test pins that scoped tokens are refused by
the effecting job routes (retry/rerun stay human-only, STUDIO-AGENT-001).
Filesystem-writing tests (logs/artifacts) use a fresh app per test: the
shared app's data_dir is session-scoped and job-id-derived paths would
collide with leftover files across tests (see tests/conftest.py).
"""
...
@pytest.fixture
def fresh_client(client_factory):
...
def test_anonymous_callers_get_401_on_all_job_tool_endpoints(anon_client, job_db) -> None:
    _seed_workspace(job_db, "ws-any")
...
def test_full_user_session_gets_403_on_all_job_tool_endpoints(client, job_db) -> None:
    _seed_workspace(job_db, "ws-any")
...
def test_scoped_token_passes_scope_guard_on_all_job_tool_endpoints(client, job_db) -> None:
...
def test_workspace_bound_token_is_refused_on_other_workspaces(client, job_db) -> None:
    ...
    bound, _ = _scoped_client(client, job_db, workspace_id="ws-home")
...
def test_scoped_token_is_refused_on_job_action_endpoints(client, job_db) -> None:
...
def test_get_job_detail_trims_payload_and_suggests_rerun(client, job_db) -> None:
...
def test_get_job_detail_404_for_other_workspace_job(client, job_db) -> None:
...
def test_list_jobs_filters_status_and_caps_limit(client, job_db) -> None:
...
def test_get_node_logs_defaults_to_latest_failed_run(fresh_client) -> None:
    job_db = fresh_client.app.state.job_db
    ...
    scoped, _ = _scoped_client(fresh_client, job_db)
...
def test_get_node_logs_unknown_node_404(client, job_db) -> None:
...
def test_read_artifact_returns_trimmed_content(fresh_client) -> None:
    job_db = fresh_client.app.state.job_db
    settings = fresh_client.app.state.settings
...
def test_read_artifact_404_for_missing_name(client, job_db) -> None:
...
def test_compare_jobs_diffs_node_statuses(client, job_db) -> None:
...
def test_compare_jobs_404_when_either_job_is_elsewhere(client, job_db) -> None:
...
def test_get_job_context_binds_session_workspace_and_focus(client, job_db) -> None:
    ...
    scoped, admin_id = _scoped_client(client, job_db, workspace_id="ws-ctx")
...
def test_get_job_context_reports_other_jobs_failures_on_focus_node(client, job_db) -> None:
    ...
    scoped, admin_id = _scoped_client(client, job_db, workspace_id="ws-flaky")
...
def test_get_job_context_404_for_cross_workspace_session_or_job(client, job_db) -> None:
    ...
    bound, admin_id = _scoped_client(client, job_db, workspace_id="ws-home2")
