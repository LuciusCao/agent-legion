...
Production wiring lands in routes/__init__.py at tower integration time; until
then these tests mount the router test-locally (``_mount_job_tool_routes``,
guarded to a no-op once the production mount exists) replicating what
``secured()`` does there (``/api`` prefix + ``require_workspace_access``). The
guard matrix mirrors test_studio_agent_tools.py: anonymous 401, full session
403, scoped tokens pass the scope guard, workspace-bound tokens get 403 on
other workspaces. The action-surface test pins that scoped tokens are refused
by the effecting job routes (retry/rerun stay human-only, STUDIO-AGENT-001).
All tests use a fresh app per test: the mount mutates the app (shared-app
mutation is banned, tests/conftest.py), and the fresh app's function-scoped
data_dir keeps job-id-derived log/artifact files from colliding across tests.
"""
...
_JOBS_ROUTE_PATH = "/api/studio-agent/tools/workspaces/{workspace_id}/jobs"


def _mount_job_tool_routes(client) -> None:
    """Replicate the production ``secured()`` wiring test-locally — inserted
    BEFORE the ``/api/studio-agent`` MCP ASGI mount, which is appended at app
    build time and would otherwise shadow the late-mounted routes (prefix
    match → its 401). No-op once the production mount (routes/__init__.py,
    tower integration) already serves the routes."""
    from starlette.routing import Mount

    app = client.app
    if any(getattr(route, "path", "") == _JOBS_ROUTE_PATH for route in app.router.routes):
        return
