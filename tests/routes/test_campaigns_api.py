...
Pins the endpoint surface (create JSON / create multipart / preview / list /
detail / pause / resume / cancel), the permission matrix (viewer 403 on
writes but 200 on reads, non-member 404 — anti-enumeration, studio-agent
token refused on every route of the group), and the PR-A intermediate state:
a created campaign stays ``pending`` (no feeder in this slice) and is fully
