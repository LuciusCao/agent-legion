"""Workspace API intake token boundaries (#626).

Split from tests/routes/test_workspace_api_tokens.py to stay clear of the
test-file line budget (the parent file owns the management lifecycle and the
paginated job reads). This sibling pins the machine identity's permission
boundaries and attack surface: cross-workspace refusal (404), expiry and
revocation semantics (with the watermark stamping rules from the codex3 P2
refinement), the api-scope blast radius (admin endpoints, effecting
endpoints, drafts, scopeless mounts — every route off the intake allowlist
must refuse it), the store's equal-work failure paths (timing
indistinguishability), and the legitimate neighbours the refusals must NOT
swallow (studio-agent scoped tokens, full user sessions).
"""
...
# --- auth chain: cross-workspace, expiry, revocation -----------------------------
