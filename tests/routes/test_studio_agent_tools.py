...
The workflow draft tools (read + CAS write, #633) live in the sibling
test_studio_agent_tools_drafts.py (#779 codex train review R3 split);
shared scaffolding is in tests/routes/studio_agent_tools_testlib.py.
"""
...
def _tool_endpoints(workspace_id: str) -> list[tuple[str, str, dict | None]]:
    ...
    return [
        ...
        # #633：workflow 草稿工具（读 + CAS 写）与既有草稿工具同面。
        ("GET", f"{base}/workflow/draft", None),
        ("PUT", f"{base}/workflow/draft", {"definition_yaml": "key: x\n"}),
        ...
        # keys 404, which still proves the scope guard let the token through.
        ("GET", "/api/studio-agent/tools/workspaces/wf/skills/wf/review", None),
        ("POST", "/api/studio-agent/tools/workspaces/wf/skills/wf/review/validate", None),
        (
            "POST",
            "/api/studio-agent/tools/workspaces/wf/skills/wf/review/versions",
