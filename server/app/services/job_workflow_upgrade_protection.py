...
from server.app.workflows.workflow_consumption import walk_downstream
...
def _name_indexes(
    ...
    for key, node in definition.executable_nodes.items():
        ...
        for name in node.inputs:
            consumers.setdefault(name, set()).add(key)
...
def input_protection_plan(
    ...
    *,
    ...
    input_names = {name for node in executable.values() for name in node.inputs}
