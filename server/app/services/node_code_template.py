...
Node SDK (``workspace_libs.node_sdk``) and is served read-only by the node
code route. It must stay directly runnable: a model or human who has read the
SDK fills in the business logic without touching the scaffolding.
"""
...
Entry contract: a module-level ``run`` taking the SDK NodeContext (the
``@entrypoint`` decorator adapts it to the executor's run(job, job_dir,
runtime) contract, EXEC-CODE-002). Use the Node SDK instead of hand-rolling
JSON IO or config merging. Custom code runs inside the velites OS sandbox
