...
from review import validate_review_result  # type: ignore[import-not-found]
...
def validate(job_dir: Path) -> list[str]:
    ...
    # Re-validate distractors_raw.json structure
    try:
    ...
    # Validate review report and reviewed artifact using shared review logic
    try:
        validate_review_result(
            source_path=job_dir / "distractors_raw.json",
            reviewed_path=job_dir / "distractors_reviewed.json",
            report_path=job_dir / "distractors_review_report.json",
            exact_copy=True,
        )
    except ContractError as exc:
        errors.append(str(exc))
