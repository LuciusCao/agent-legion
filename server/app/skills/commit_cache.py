...
Governance: bounded per skill — the cache keeps the most recent
...
runs-dir sweeper never enters ``.shared`` (runs_gc skips it), and per-
validation cleanup is gone: validators read the shared tree read-only, so
``SkillManager.cleanup_execution`` is no longer part of the validation
path. Eviction under a live reader is bounded away by the LRU touch (an
actively used commit is always the freshest) and by eviction only running
when a NEW commit materializes.
"""
...
def materialized_commit_dir(manager: SkillManager, skill_key: str, commit: str) -> Path:
    """Return the shared cached materialization of (``skill_key``, ``commit``).

    Hit (marker file present): pure path probe, zero git calls. Miss:
    serialize on the per-repo FileLock, export the commit's tree to a temp
    sibling, atomically rename into place, then drop the integrity marker.
    Safe to call from pool workers — everything it touches is filesystem-
    level (the FileLock) and process-independent.
    """
