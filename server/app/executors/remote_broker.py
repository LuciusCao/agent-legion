...
from server.app.executors._remote_worker_registry import WorkerRegistryStore
...
class RemoteExecutionBroker:
    ...
    def __init__(
        ...
        *,
        ...
        self._dispatcher = CompletionDispatcher()
        self._workers = WorkerRegistryStore(db_path, self._now)
    ...
    def cancel_by_lease(self, lease_id: str) -> bool:
        """Cancel the queued/claimed execution backing ``lease_id``, if any."""
        with read_connection(self._db_path) as conn:
            row = conn.execute(
                "select execution_id from remote_executions"
                " where lease_id = ? and state in ('queued', 'claimed')",
                (lease_id,),
            ).fetchone()
        if row is None:
            return False
        self.cancel(str(row["execution_id"]))
        return True
    ...
    def wait_idle(self) -> None:
        """Block until queued completion callbacks have run."""
        self._dispatcher.wait_idle()

    def close(self) -> None:
        """Drain and stop the completion dispatcher."""
    ...
    def register_worker(
        ...
        self._workers.register(worker_id, name, capabilities, slots)

    def touch_worker(self, worker_id: str) -> None:
        self._workers.touch(worker_id)
    ...
    def update_worker_labels(self, worker_id: str, labels: Mapping[str, Any]) -> None:
        ...
        """
        self._workers.update_labels(worker_id, labels)

    def list_workers(self) -> list[dict[str, Any]]:
        return self._workers.list_workers()
    ...
    def issue_worker_token(
        ...
        return self._workers.issue_token(worker_id, name, capabilities, slots, labels)

    def authenticate_worker(self, token: str) -> dict[str, Any] | None:
        return self._workers.authenticate(token)

    def revoke_worker(self, worker_id: str) -> bool:
        return self._workers.revoke(worker_id)

    def is_worker_revoked(self, worker_id: str) -> bool:
        return self._workers.is_revoked(worker_id)
    ...
    def _publish_completion(self, execution_id: str) -> None:
        ...
        for callback in self._completion_callbacks:
            self._dispatcher.dispatch(callback, execution_id, outcome)
