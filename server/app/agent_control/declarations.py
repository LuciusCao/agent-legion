"""Bounded normalization for Worker scheduling declarations."""
...
def normalize_models(
    ...
    *,
    ...
    for item in values:
        ...
        if require_runtime and not runtime:
            raise ValueError("protocol v3 model declarations require runtime")
        if (
            not provider
            or not model
            or provider == "*"
        ...
        targets = [runtime] if runtime else list(runtimes)
        if any(target not in runtimes and target != "*" for target in targets):
            raise ValueError("worker model runtime must be one of the declared runtimes")
        normalized.update((target, provider, model) for target in targets)
    return [
        {"runtime": runtime, "provider": provider, "model": model}
        for runtime, provider, model in sorted(normalized)
    ]
...
def normalize_worker_declarations(
    ...
    *,
    ...
    """Normalize HTTP declarations, retaining wildcard for legacy direct callers."""
    ...
    normalized_models = (
        ...
        if models is None
        else normalize_models(models, runtimes, require_runtime=require_model_runtime)
    )
