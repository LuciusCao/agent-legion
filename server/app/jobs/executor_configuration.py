...
def get_workspace_executor_configuration(
    ...
    allocations = conn.execute(
        ...
        "from workspace_executor_allocations where workspace_id=%s order by executor_id",
        (workspace_id,),
    ).fetchall()
    bindings = conn.execute(
        "select workflow_key, node_key, executor_id "
        "from workspace_node_bindings where workspace_id=%s order by workflow_key, node_key",
        (workspace_id,),
    ).fetchall()
...
def replace_workspace_executor_configuration(
    ...
    conn.execute("delete from workspace_node_bindings where workspace_id=%s", (workspace_id,))
    conn.execute(
        "delete from workspace_executor_allocations where workspace_id=%s", (workspace_id,)
    )
    conn.executemany(
    ...
    conn.executemany(
        ...
        "(workspace_id, workflow_key, node_key, executor_id) values (%s, %s, %s, %s)",
        [
