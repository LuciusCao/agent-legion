...
def _build_boto3_client(settings: S3Settings, *, endpoint_override: str | None = None) -> Any:
    ...
    # Bounded timeouts (retries off — the upload path has its own bound) plus
    # SigV4 pinned (#340): boto3 presign falls back to SigV2 on custom
    # endpoints, which SeaweedFS's gateway rejects (403); V4 is universally
    # accepted, so pinning is safe everywhere.
    config = Config(
        connect_timeout=10, read_timeout=120, retries={"max_attempts": 1}, signature_version="s3v4"
    )
