...
# config/agent_legion.yaml (the asr section retired with the legacy business
# transcription pipeline — business parameters and machine paths now live in
# the corresponding node configuration; the openclaw section retired later
# with the openclaw runtime itself, #75). No split file owns any top-level
# key anymore; the canonical layout loads zero files and starts from the
# code defaults plus env overrides.
...
_RETIRED_FILE_GUIDANCE = {
    ...
    "agent_legion.yaml": "config/agent_legion.yaml was retired. Migrate: asr.provider / asr.timeout_seconds and ASR machine paths -> the corresponding node configuration in Studio (the ASR env channel retired with the legacy transcription pipeline); openclaw -> retired with the openclaw runtime (#75), no replacement needed; then delete config/agent_legion.yaml.",  # fmt: skip
}
