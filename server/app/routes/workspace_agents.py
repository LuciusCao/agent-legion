...
def create_workspace_agents_router(
    service: WorkspaceAgentAssignmentService, settings: Settings
) -> APIRouter:
    router = APIRouter()

    @router.get("/workspaces/{workspace_id}/agents", response_model=WorkspaceAgentListResponse)
        ...
        try:
            return WorkspaceAgentListResponse(
                root=[
                    ...
                    for assignment in service.list_assignments(workspace_id)
                ]
            )
    ...
    def set_workspace_agent(
        workspace_id: str, config: WorkspaceAgentConfig
    ) -> WorkspaceAgentAssignmentResponse:
        require_pipelines_enabled(settings)
        try:
