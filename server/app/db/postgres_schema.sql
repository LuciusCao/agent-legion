...
-- Workflow catalog (schema v40): DB-backed registry of known workflow keys.
-- Built-in rows are upserted from workflows.builtin at startup (seed source
-- stays in the repo); registered rows come from the admin API and start
-- without a definition (first workspace draft publish creates revision v1).
create table if not exists workflow_catalog (
  key text primary key,
  label text not null,
  description text not null default '',
  origin text not null check(origin in ('builtin', 'registered')),
