"""Per-candidate claim submission for the workflow worker's ready queues.

Split from ``schedule`` for the file-size budget: ``schedule`` keeps the
round-robin drain, this module owns the single-candidate dispatch decision
(approval park / shard fan-out / agent enqueue / remote code Worker / local
code pool, in that order). The capacity snapshot checks here are
optimization hints that skip pointless ``try_claim`` write-lock
acquisitions; the lease claim transaction remains the authoritative
...
def try_claim_and_submit(
    ...
    *,
    ...
    if node.node_type == APPROVAL_NODE_TYPE:
        return worker.leases.park_awaiting_approval(
            job["id"], node_key, execution_generation=execution_generation
        )
