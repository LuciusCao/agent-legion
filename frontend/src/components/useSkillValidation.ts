...
/** 校验的绑定上下文：value（当前绑定 key）+ nodeKey（发起校验的检查器
 * 节点身份，宿主传入）。value 相同而节点不同（A、B 都未绑定）时仍是
 * 不同上下文（codex 三轮 P1 on #427）。 */
...
export const sameContext = (a: SkillValidationContext, b: typeof a) =>
