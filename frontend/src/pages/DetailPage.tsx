    ...
    <section className="view detail-view">
      <section className="detail-upper">
        <header className="detail-topbar">
          ...
          <div className="detail-title-block" data-tooltip={hook.detailTitle}>
            ...
            {hook.video && (
              <p className="detail-meta-line">
                <span>{TYPE_LABELS[hook.video.content_type]} · {hook.video.external_id || "未填 ID"}</span>
                <span className={`status-badge ${statusGroup(hook.video)}`}>
                  {STATUS_LABELS[statusGroup(hook.video)] || hook.video.status}
                </span>
                {hook.video.content_type === "knowledge" && hook.video.status === "completed" && (
                  <InteractionReviewBadge status={hook.video.interaction_review_status} />
                )}
                {!!hook.video.packed && <span className="packed-badge">已打包</span>}
              </p>
            )}
            {hook.video?.error_message && (
              <p className="error-text" style={{ marginTop: 4 }}>{hook.video.error_message}</p>
            )}
          </div>
          <div className="detail-actions">
            ...
            <md-icon-button onClick={() => hook.setRunToDialogOpen(true)} title="运行到">
              <md-icon>play_circle</md-icon>
            </md-icon-button>
            <md-icon-button
              disabled={(!hook.video || hook.video.status !== "completed") || undefined}
              onClick={hook.handlePackage}
              title="打包"
            >
              <md-icon>inventory_2</md-icon>
            </md-icon-button>
            <md-icon-button style={{ color: "var(--md-sys-color-error)" }} onClick={hook.openDeleteDialog} title="删除">
              <md-icon>delete</md-icon>
            </md-icon-button>
            <md-icon-button id="more-menu-btn" onClick={() => hook.setMoreDialogOpen(true)} title="更多">
      ...
      {hook.moreDialogType === "subtitles" && (
        ...
        >
          ...
          <div slot="content" style={{ maxHeight: "60vh", overflow: "auto", padding: "8px 0" }}>
            <SubtitlePanel currentTime={hook.currentTime} onSeek={(time) => {
              hook.handleSeek(time);
              hook.closeMoreDialog();
            }} />
          </div>
