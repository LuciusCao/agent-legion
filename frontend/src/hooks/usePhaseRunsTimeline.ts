...
export interface UsePhaseRunsTimelineReturn {
  ...
  setSessionDialog: (
    v: { runId: number; sessionId: string; videoId: string } | null
  ) => void
...
function buildItem(
  ...
  const prevFinished = prevRun?.finished_at
    ? new Date(prevRun.finished_at).getTime()
    : null
...
export function usePhaseRunsTimeline(
  ...
  const allPhases =
    contentType === 'question' ? QUESTION_PHASES : KNOWLEDGE_PHASES
  ...
  const { latestItems, historyItems } = useMemo(() => {
    ...
    const currentPhaseIndex = currentPhase
      ? allPhases.indexOf(currentPhase)
      : -1
    const latestPhaseIndex =
      currentPhaseIndex >= 0
        ? currentPhaseIndex
        : latestRun
          ...
          : -1
    const currentPhases =
      latestPhaseIndex >= 0 ? allPhases.slice(0, latestPhaseIndex + 1) : []
      ...
      if (run) {
        const prev =
          latestItems.length > 0
            ? latestItems[latestItems.length - 1].run
            : null
        latestItems.push(
          buildItem(
            run,
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      historyItems.push({
        ...buildItem(run, prev, now, transcriptionRuns),
        occurrence: count,
      })
    }
  ...
  async function openSession(run: PhaseRun, sessionId: string) {
    ...
    try {
      const data = await api<{ log: string }>(
        `/api/videos/${run.video_id}/phase-runs/${runId}/session`
      )
