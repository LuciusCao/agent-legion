...
describe('useWorkspaceMutations', () => {
  ...
  it('createWorkspace calls the API with the explicit id and invalidates the list', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 'ws-new', name: '新空间' })
    })

    expect(mockCreateWorkspace).toHaveBeenCalledWith('ws-new', '新空间')
  ...
  it('createWorkspace propagates API errors', async () => {
    mockCreateWorkspace.mockRejectedValue(new Error('duplicate id'))
    const { result } = renderHook(() => useCreateWorkspace(), { wrapper })

    await expect(
      result.current.mutateAsync({ id: 'ws-dup', name: '重复' })
