...
describe('computeProgress', () => {
  it('returns 1 for completed videos', () => {
    expect(
      computeProgress(video({ status: 'completed', current_phase: 'assemble' }))
    ).toBe(1)
  })
  ...
  it('returns 0 for waiting_for_url', () => {
    expect(
      computeProgress(
        video({ status: 'missing_url', current_phase: 'waiting_for_url' })
      )
    ).toBe(0)
  })
  ...
  it('computes knowledge progress correctly', () => {
    ...
    expect(
      computeProgress(video({ status: 'queued', current_phase: 'download' }))
    ).toBe(0)
    // running at download -> 0.5/7
    expect(
      computeProgress(video({ status: 'running', current_phase: 'download' }))
    ).toBe(0.5 / 7)
    // queued at transcribe -> 1/7
    expect(
      computeProgress(video({ status: 'queued', current_phase: 'transcribe' }))
    ).toBe(1 / 7)
    // running at assemble -> 6.5/7
    expect(
      computeProgress(video({ status: 'running', current_phase: 'assemble' }))
    ).toBe(6.5 / 7)
  })
  ...
  it('returns 0 for unknown phase', () => {
    expect(
      computeProgress(video({ status: 'queued', current_phase: 'unknown' }))
    ).toBe(0)
  })
})
...
describe('run-to phase helpers', () => {
  ...
  it('excludes terminal videos from continue-to eligibility', () => {
    expect(
      canContinueTo(
        video({ status: 'failed', current_phase: 'transcribe' }),
        'assemble'
      )
    ).toBe(false)
    expect(
      canContinueTo(
        video({ status: 'completed', current_phase: 'chapter_generate' }),
        'assemble'
      )
    ).toBe(false)
  })
