...
- velites（`velites/` crate，自研 Rust harness）：pi、openclaw、velites 是平级
  ...
  钉死 velites 实现、忽略 flavor。新增 agent 一律直接声明目标 runtime，不要
  依赖 flavor 路径；pi 退役另立项。Workflow 节点不感知 runtime/harness 实现。
  Worker 声明 velites 前必须先在 PATH 提供 velites 二进制（启动预检缺失即拒启动）。
