...
- Storage paths are persisted as **relative POSIX paths** under `settings.data_dir` (canonical prefixes: `videos/`, `jobs/`, `logs/`, `packages/`). The API projects absolute paths when returning values to clients. See `server/app/storage_paths.py` and migration `v009_relative_path_storage.py`.

## Pi / Agent Legion Skills

The Agent Legion workflow executes capability nodes through the Pi CLI (`@earendil-works/pi-coding-agent`).
Pi skills are **not** repository-owned; they live in standalone git repositories managed by `SkillManager`.

### Skill locations

- Runtime checkout cache: `server/app/skills/manager.py` resolves `config/skills.yaml` sources into isolated per-execution copies.
- Source registry: `config/skills.yaml` declares skill repos (local paths or git URLs).
...
The script is idempotent. Use `--push` and `--remote-template` to publish to a remote after the initial commit.
...
### `config/skills.yaml` and `config/skills.lock`

- `skills.yaml` — registry of external skill repositories. Each entry declares `workflow`, `capability`, `source` (git URL or local path), and optional `ref`.
- `skills.lock` — auto-generated commit lockfile. Commit it so every environment resolves the same skill version.
- `SkillManager` validates paths, clones/checks out the declared ref, and returns a safe skill directory for each execution.
