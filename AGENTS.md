...
- 「先 ensure-velites 再跑单测」是开发机常态（dev 栈/验收构建过 `data/bin/velites`）——worker 测试要隔离自带二进制目录时必须 patch 事实源 `code_sandbox.BUNDLED_SANDBOX_DIR`（re-export 侧的 `binary_resolution.BUNDLED_BINARY_DIR` 打不到解析函数的读取点，#496），否则干净 CI 全过、本机却静默变红。
