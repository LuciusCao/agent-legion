...
- 生产 worktree（如 `.worktrees/prod`）禁止 debug 与改代码：只允许 `git pull` 与 `make prod-up` / `make prod-down` / `make prod-restart`（原子重启，#629；prod-up 经 `scripts/ensure-velites.sh` 自动重建过期 velites 二进制）。服务生命周期命令只能由人在终端执行——Studio 会话 agent 的 terminal 对其硬拒绝（`server/app/studio_chat/terminal_guard.py`）。所有修复与调试必须在 develop worktree 进行，经 PR → main → prod pull 到达生产。生产命令只在 prod worktree 跑，在其他 worktree 跑会抢生产端口并连错数据库。
...
- 运行 `make install-hooks` 启用版本化本地门禁：pre-commit 跑 fast gate，pre-push 默认跑 smoke 级（成员见 `tests/conftest.py`）并按推送路径裁剪 lane（纯前端跳过 backend pytest、纯 `velites/` 只跑 rust、docs 只跑静态、共享文件/新分支全量）。用 `AGENT_LEGION_GATE_LEVEL=quick` / `full` 升级单次推送。CI full gate 按同样的路径规则裁剪 lane（检测逻辑见 workflow 的 `changes` job；docs-only 变更仅运行 changes 与 docs-terms 轻量门禁，其余 lane 跳过，由唯一 required context `quality-gate` 聚合裁决）；ci-extended 与 nightly-e2e 在 `.github/workflows/nightly-gate.yml` 独立执行。
